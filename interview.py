...
if not st.session_state.messages:
    ...
    if api == "openai":
        ...
        with st.chat_message("assistant", avatar=config.AVATAR_INTERVIEWER):
            # Stream OpenAI message
            stream = client.chat.completions.create(**api_kwargs)
            message_interviewer = st.write_stream(stream)

        # Save the streamed message
        st.session_state.messages.append(
            {"role": "assistant", "content": message_interviewer}
        )

    elif api == "anthropic":
        # Add initial dummy user message (Anthropic requires it)
        ...
        with st.chat_message("assistant", avatar=config.AVATAR_INTERVIEWER):
            ...
            with client.messages.stream(**api_kwargs) as stream:
                for text_delta in stream.text_stream:
                    ...
                    # Display the live typing with RTL direction
                    message_placeholder.markdown(
                        f'<div style="direction: rtl; text-align: right;">{message_interviewer}▌</div>',
                        unsafe_allow_html=True
                    )

            # Display the final message cleanly (without ▌)
            message_placeholder.markdown(
                f'<div style="direction: rtl; text-align: right;">{message_interviewer}</div>',
                unsafe_allow_html=True
            )

        # Save final message to session state
        st.session_state.messages.append(
            {"role": "assistant", "content": message_interviewer}
        )
